...
- 메모는 선택한 인덱스 위치에서 패널 크기로 펼쳐지고, 접을 때는 같은 위치로 다시 수축됩니다. 선택 인덱스는 패널이 다 접힐 때까지 남아 두 모션을 자연스럽게 이어 줍니다.
- PEEK 메모의 제목줄이나 본문을 클릭해도 즉시 ICE로 전환되어 TopBar와 편집 포커스가 활성화됩니다.
...
- `ICE`에서만 나타나는 한 줄 Crepe 상단 툴바. 좁은 패널에서는 툴바 안쪽만 가로 스크롤됩니다.
- TopBar와 본문은 서로 다른 세로 영역을 사용합니다. 긴 문서도 TopBar 아래 본문 영역에서만 스크롤되며 오른쪽 스크롤바는 표시하지 않습니다.
...
별도 네이티브 하단 툴바는 두지 않습니다. 기본 본문 글꼴은 11px이며 설정에서 조절할 수 있습니다. 밑줄, 글자색, 정렬은 각각 `<u>`, 안전한 6자리 색상 `<span>`, 정렬 `<div>`로 Markdown에 왕복 저장합니다. 지원하지 않는 raw HTML은 텍스트형 보존 노드로 유지해 실행은 막고 원문은 지우지 않습니다. raw Markdown 모드와 AI 기능은 제공하지 않습니다.

제목과 본문에는 전역 글자 외곽선을 적용하지 않습니다. `# + Space`로 제목 블록을 만들더라도 다른 본문 글자에 stroke가 번지지 않습니다.
Heading 위쪽 간격은 Crepe 기본값보다 작게 조정해 문단 사이가 과도하게 벌어지지 않습니다.
